    ...
    <Card className="bg-white border-gray-200 shadow-sm hover:shadow-lg transition-shadow">
      <CardHeader>
        <div className="flex justify-between items-start flex-wrap gap-4">
          ...
          <Badge variant="secondary" className="bg-gray-100 text-gray-800 border border-gray-300">{experience.duration}</Badge>
        </div>
      </CardHeader>
      <CardContent>
        ...
        {experience.technologies && (
          <div className="mt-4 flex flex-wrap gap-2">
            {experience.technologies.map((tech, index) => (
              <Badge key={index} variant="outline" className="border-gray-400 text-gray-700 text-xs">
                {tech}
              </Badge>
            ))}
          </div>
